...
public class DeleteFunctionBean extends AccountsRequestBean{
						...
						if(lstRoleTarget != null){
							...
							while(roleTargetIterator.hasNext()){
								...
								if(roleTarget.getApplicationName().equalsIgnoreCase("IBM MAINFRAME RACF") &&
									...
									logger.debug(className + "matched user id " + linkApp.getUserId() + " with role " + rAssigment.getRoleName());
									LinkRole linkRole = new LinkRole(rAssigment.getRoleName(), rAssigment.getAssignmentId());
									tempList.add(linkRole);
								}
								
							}
							try {
								logger.debug(className + " Native role target : " + roleTarget.getNativeIdentity());
								List exceptions = identity.getExceptions();
								Iterator EntitlementGroupIterator =  exceptions.iterator();
								
								while(EntitlementGroupIterator.hasNext()) {
									EntitlementGroup enttGroup = (EntitlementGroup)EntitlementGroupIterator.next();
									logger.debug(className + " entitlement group xml : " + enttGroup.toXml());
									Attributes attrGroup = (Attributes) enttGroup.getAttributes();
									Map attrMapGroup = attrGroup.getMap();
									List lstGroup = (List) attrMapGroup.get("groups");
									String group = lstGroup.toString().replace("[", "").replace("]", "");
									logger.debug(className + "size group list : " + lstGroup.size() );
									if(enttGroup.getDisplayName().equalsIgnoreCase(roleTarget.getNativeIdentity())) {
										LinkGroup linkGroup = new LinkGroup(enttGroup.getDisplayName(), enttGroup.getId(), group);
	...
	private List getAdMainframeLinks() throws GeneralException{
		...
		while(it.hasNext()){
			...
			if(CommonUtil.AD_APPLICATION.equalsIgnoreCase(appsName) || CommonUtil.RACF_APPLICATION_NAME.equalsIgnoreCase(appsName)){
...
public class DeleteFunctionBean extends AccountsRequestBean{
	...
	public DeleteFunctionBean() {
		...
		try {
			...
			List lstRole = null;
			Iterator tempLinkIterator = null;
			EntitlementGroup enttGroup = null;
			Attributes attrGroup = null;
			Map attrMapGroup = null;
			List lstGroup = null;
			LinkApplication linkApp = null;
			List tempList = new ArrayList();
			List tempList2 = new ArrayList();
			Iterator iteratorRole = null;
			RoleAssignment rAssignment =  null;
			List lstRoleTarget = null;
			RoleTarget roleTarget = null;
		
				while(EntitlementGroupIterator.hasNext()) {
					enttGroup = (EntitlementGroup)EntitlementGroupIterator.next();
					attrGroup = (Attributes) enttGroup.getAttributes();
					attrMapGroup = attrGroup.getMap();
					lstGroup = (List) attrMapGroup.get("groups");
			...
			if(lstTemp != null && preferenceMap != null){
				...
				while(tempLinkIterator.hasNext() && lstRole != null){
					...
					linkApp = (LinkApplication)tempLinkIterator.next();
							...
							while(iteratorRole.hasNext()){
								...
								if(lstRoleTarget != null){
									Iterator roleTargetIterator = lstRoleTarget.iterator();
									
									while(roleTargetIterator.hasNext()){
										
										roleTarget = (RoleTarget)roleTargetIterator.next();
										if(roleTarget.getApplicationName().equalsIgnoreCase("IBM MAINFRAME RACF") && roleTarget.getNativeIdentity().equalsIgnoreCase(linkApp.getUserId())){
											
											logger.debug(className + "matched user id " + linkApp.getUserId() + " with role " + rAssignment.getRoleName());
											LinkRole linkRole = new LinkRole(rAssignment.getRoleName(), rAssignment.getAssignmentId());
											tempList.add(linkRole);			
										}
									}
								}
					}
					logger.debug(className + "total list user : " + tempList.size());
					if(tempList != null && tempList.size() >= 1){
						logger.debug(className + " fungsi lebih dari 1, dengan account " + linkApp.getUserId());
						linkApp.setLstEntGroup(tempList2);
						linkApp.setLstRoles(tempList);
				...
				logger.debug(className + "entry map entitlement :" + attrMapGroup.toString());
			}
			
			if(enttGroup.getNativeIdentity().equalsIgnoreCase(roleTarget.getNativeIdentity())) {
				LinkGroup linkGroup = new LinkGroup(enttGroup.getNativeIdentity(), enttGroup.getId(), group);
				tempList2.add(linkGroup);
			}	
		logger.debug(className + " list yang ditampilkan : " + lstAccounts.size());	
		setAccountFunctionList(lstAccounts);			
	...
	private List getAdMainframeLinks() throws GeneralException{
		...
		while(it.hasNext()){
			Link link = (Link)it.next();
			logger.debug(className + "link identity : " + link.getNativeIdentity());
			String appsName = link.getApplicationName();
			if(CommonUtil.AD_APPLICATION.equalsIgnoreCase(appsName) || CommonUtil.RACF_APPLICATION_NAME.equalsIgnoreCase(appsName)){
				LinkApplication linkApps = new LinkApplication();
				linkApps.setApplicationName(appsName);
		...
		if(getAccountFunctionList() != null){
			...
			while(i.hasNext()){
				LinkApplication linkApps = (LinkApplication)i.next();
					...
					logger.debug(className + methodName + " account checked " + linkApps.getUserId());
	...
	private void invokeWorkflow(SailPointContext context, Identity employee, List targetApplication) throws GeneralException{
		...
		while(it.hasNext()){
			...
			HashMap launchArgsMap = WorkflowUtil.getLaunchArgsMap(employee, getLoggedInUser(), plan, BCAApproval.FLOW_TYPE_PENGURANGAN);
