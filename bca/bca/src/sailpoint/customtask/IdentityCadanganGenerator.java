...
public class IdentityCadanganGenerator extends AbstractTaskExecutor{
		...
		for(int i=1; i<rows; i++){
		 ...
		 for(int i=0; i<al.size(); i++){
			 	...
			 	if(oldkPIdentity == null){
					...
					identity.setPassword("bcabca" + al.get(i));
			...
			for(int i=0; i<listLength; i++){
				...
				if(branchCode != "0988"){
					...
					if(wapim != null && oldIdentity == null){
						...
						identity.setPassword("bcabca" + branchCode);
						try{
						...
						/*if(oldIdentity.getAttribute(IdentityAttribute.DIVISION_CODE) == null || oldIdentity.getAttribute(IdentityAttribute.DIVISION_CODE).toString().contains("0000"))
						{
							oldIdentity.setAttribute(IdentityAttribute.DIVISION_CODE, divisionCode);
						}
						else */if(oldIdentity.getAttribute(IdentityAttribute.FIRST_NAME) == null)
						{
